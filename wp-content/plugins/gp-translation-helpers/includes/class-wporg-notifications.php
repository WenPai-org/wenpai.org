...
class WPorg_GlotPress_Notifications {
	...
	/**
	 * Indicates if the given user is a GTE at translate.wenpai.org.
	 *
	 * @since 0.0.2
	 *
	...
	/**
	 * Gets the opt-in/oup-out message to show at the bottom of the discussions at translate.wenpai.org.
	 *
	 * @param int $original_id The id of the original string used for the discussion.
	 *
	 * @return string The message to show at the bottom of the discussions at translate.wenpai.org.
	 */
	public static function optin_message_for_each_discussion( int $original_id ): string {
		...
		if ( self::is_global_optout_email_address( $user->user_email ) ) {
			...
			$output .= ' <a href="https://translate.wenpai.org/settings/">' . __( 'Start receiving notifications.' ) . '</a>';
			return $output;
		}
		if ( GP_Notifications::is_user_opt_out_in_discussion( $original_id, $user ) ) {
			...
			$output .= ' <a href="https://translate.wenpai.org/settings/">' . __( 'Stop receiving notifications.' ) . '</a>';
			return $output;
		}
