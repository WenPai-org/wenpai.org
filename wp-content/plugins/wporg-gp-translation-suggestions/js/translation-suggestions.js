( function( $ ){
	function fetchSuggestions( $container, apiUrl, originalId, nonce ) {
		...
		xhr.done( function( response ) {
			...
			if ( response.success ) {
				...
				$container.append( $( '<span/>', { 'text': '加载翻译建议失败！' } ) );
			}
	...
	function maybeFetchTranslationAISuggestions() {
		var $container = $gp.editor.current.find( '.suggestions__translation-ai' );
		if ( !$container.length ) {
			return;
		}
		...
		fetchSuggestions( $container, window.WPORG_TRANSLATION_AI_API_URL, originalId, nonce );
	}
	...
	$gp.editor.show = ( function( original ) {
		return function() {
			...
			maybeFetchTranslationAISuggestions();
