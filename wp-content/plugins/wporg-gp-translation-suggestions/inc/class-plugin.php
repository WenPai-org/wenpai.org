...
use GP_Locales;
use GP_Translation;
use WordPressdotorg\GlotPress\TranslationSuggestions\Command\Sync_Memory;
...
class Plugin {
	...
	public function plugins_loaded() {
		...
		// wporg 本来是禁止了该这仨钩子在命令行下执行，但因为平台的 corn 是由命令行触发的，所以必须放开条件
		//if ( 'cli' !== PHP_SAPI ) {
	...
	public function register_routes() {
		...
		GP::$router->prepend( "/$set/-get-ai-suggestions", [
			__NAMESPACE__ . '\Routes\Translation_AI',
			'get_suggestions'
		] );
	...
	public function pre_tmpl_load( $template, $args ) {
		...
		wp_add_inline_script(
			...
			sprintf(
				...
				wp_json_encode( gp_url_project( $args['project'], gp_url_join( $args['locale_slug'], $args['translation_set_slug'], '-get-ai-suggestions' ) ) )
			)
		);
	}
	...
	public function extend_translation_suggestions( $entry ) {
        ...
        <details open class="suggestions__translation-memory"
            ...
            <summary>来自记忆库的建议</summary>
            <p class="suggestions__loading-indicator">
                加载中，请稍等片刻...
                <span aria-hidden="true" class="suggestions__loading-indicator__icon">
                    <span></span><span></span><span></span>
                </span>
            </p>
        </details>
        <details open class="suggestions__translation-ai"
                 data-nonce="<?php echo esc_attr( wp_create_nonce( 'translation-ai-suggestions-' . $entry->original_id ) ); ?>">
            <summary>来自 AI 的建议</summary>
            <p class="suggestions__loading-indicator">
                生成中，请稍等片刻...
                <span aria-hidden="true" class="suggestions__loading-indicator__icon">
                    <span></span><span></span><span></span>
