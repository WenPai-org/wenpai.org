...
require_once WPMU_PLUGIN_DIR . '/rosetta-roles/cross-locale-pte.php';
...
class Plugin {
	 ...
	 * @var string
	 */
	public static $cache_group = 'plat-translate';
	...
	public function plugins_loaded() {
		$GLOBALS['wpdb']->plat_translation_editors = self::TRANSLATION_EDITORS_TABLE;
		...
		add_filter( 'gp_translation_set_import_status', array(
			'Cross_Locale_PTE',
	...
	public function pre_can_user( $verdict, $args ) {
		...
		// 只有管理员可以删除
		if ( 'delete' === $args['action'] && ! current_user_can( 'manage_options' ) ) {
			return false;
		}

		// 禁止在根项目上新建项目
		if ( 'write' === $args['action'] && empty( $args['parent_project_id'] ) && empty( $args['object_id'] ) ) {
			return false;
		}
		...
		if ( ! in_array( $args['object_type'], array(
			'project|locale|set-slug',
			'translation-set',
			'translation',
			'project'
		), true ) ) {
	...
	public function is_plugin_or_theme_project( $project_id ) {
		...
		return ( 0 === strpos( $project->path, 'plugins/' ) || 0 === strpos( $project->path, 'themes/' ) );
	}
	...
	/**
	 * Determines if a given user is a Admin.
	 *
	...
	public function get_blog_prefix( $locale_slug ) {
		...
		if ( ! isset( $ros_locale_assoc ) ) {
			$ros_locale_assoc = $wpdb->get_results( 'SELECT locale, subdomain FROM plat_locales', OBJECT_K );
		}
