...
class Translators extends WP_List_Table {
	...
	public function prepare_items() {
		...
		$translators = $wpdb->get_col(
			"SELECT DISTINCT(user_id) FROM {$wpdb->plat_translation_editors}"
		);
	...
	public function no_items() {
		_e( 'No translators were found.', 'rosetta' );
	}
	...
	protected function get_bulk_actions() {
		return array(
			'remove-translators' => _x( 'Remove', 'translator', 'rosetta' ),
		);
	}
	...
	public function column_cb( $user ) {
		if ( $this->user_can_promote ) {
			?>
			<label class="screen-reader-text" for="cb-select-<?php echo $user->ID; ?>"><?php _e( 'Select translator', 'rosetta' ); ?></label>
	...
	public function column_locales( $user ) {
		...
		$locales = $wpdb->get_col( $wpdb->prepare(
			"SELECT DISTINCT(locale) FROM {$wpdb->plat_translation_editors} WHERE user_id = %d"
		, $user->ID ) );
