...
class WPorg_GP_Project_Stats {
	...
	function __construct() {
		...
		if ( ! wp_next_scheduled ( 'plat_gp_stats_cache_waiting_strings' ) ) {
			wp_schedule_event( time(), 'twicedaily', 'plat_gp_stats_cache_waiting_strings' );
		}
		add_action( 'plat_gp_stats_cache_waiting_strings', [ $this, 'cache_wp_themes_wp_plugins_strings' ] );
	...
	function get_project_translation_counts( $project_id, $locale, $locale_slug, &$counts = array() ) {
		...
		$projects = GP::$project->get( $project_id );
		if ( $projects ) {
			foreach ( $projects->sub_projects() as $project ) {
				if ( ! $project->active ) {
					continue;
				}
	...
	public function cache_wp_themes_wp_plugins_strings() {
		...
		$cached_projects = [
			GP::$project->by_path( 'plugins' ),
			GP::$project->by_path( 'themes' ),
			GP::$project->by_path( 'core' ),
		];
