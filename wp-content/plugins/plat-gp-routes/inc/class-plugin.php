...
class Plugin {
	...
	public const CACHE_GROUP = 'plat-translate';
	 ...
	 *  - /projects/plugins/$project
	 *  - /projects/plugins/$project/contributors
	 *  - /projects/plugins/$project/language-packs
	 *  - /projects/themes/$project
	 *  - /projects/themes/$project/contributors
	 *  - /projects/themes/$project/language-packs
	 */
	public function register_routes() {
		...
		if ( gp_startswith( $request_uri, '/' . GP::$router->api_prefix . '/' ) ) { // API requests.
			...
			GP::$router->prepend( '/projects/plugins/?', array( __NAMESPACE__ . '\Routes\Redirector', 'redirect_index' ) );
			GP::$router->prepend( '/projects/themes/?', array( __NAMESPACE__ . '\Routes\Redirector', 'redirect_index' ) );
			...
			// Project routes.
			GP::$router->prepend( "/projects/plugins/$project", array( __NAMESPACE__ . '\Routes\WP_Plugins', 'get_plugin_projects' ) );
			GP::$router->prepend( "/projects/plugins/$project/contributors", array( __NAMESPACE__ . '\Routes\WP_Plugins', 'get_plugin_contributors' ) );
			GP::$router->prepend( "/projects/plugins/$project/language-packs", array( __NAMESPACE__ . '\Routes\WP_Plugins', 'get_plugin_language_packs' ) );
			GP::$router->prepend( "/projects/themes/$project", array( __NAMESPACE__ . '\Routes\WP_Themes', 'get_theme_projects' ) );
			GP::$router->prepend( "/projects/themes/$project/contributors", array( __NAMESPACE__ . '\Routes\WP_Themes', 'get_theme_contributors' ) );
			GP::$router->prepend( "/projects/themes/$project/language-packs", array( __NAMESPACE__ . '\Routes\WP_Themes', 'get_theme_language_packs' ) );

			// Add Robots.txt support
			GP::$router->prepend( '/robots\.txt', 'do_robots' );

			// 待审项目
			GP::$router->prepend( '/waiting-list.*', array( __NAMESPACE__ . '\Routes\Index', 'do_waiting' ) );
	...
	public static function get_translation_status() {
		...
		$translation_status = $wpdb->get_results( $wpdb->prepare(
			...
			396517, // 396517 = core/dev/body
			'default'
		), OBJECT_K );

		wp_cache_set( 'translation-status', $translation_status, self::CACHE_GROUP, 15 * MINUTE_IN_SECONDS );
