 ...
 * Locale Route Class.
 *
 * Provides the route for translate.wenpai.org/locale/$locale.
 */
class Locale extends GP_Route {

	private $cache_group = 'plat-translate';
	...
	private function get_project_icon( $project, $sub_project, $size = 128 ) {
		...
		switch ( $project->slug ) {
			...
			case 'themes':
				...
				$directory_post_id = gp_get_meta( 'project', $sub_project->id, 'directory_post_id' );
				if ( $directory_post_id ) {
					switch_to_blog( SITE_ID_THEMES );
					$screenshot = get_post_meta( $directory_post_id, 'banner', true );
					restore_current_blog();
				}

				if ( $screenshot ) {
					return '<div class="theme icon"><img src="' . $screenshot . '" width="' . $size . '" height="' . $size . '"></div>';
				} else {
			...
			case 'plugins':
				...
				if ( $directory_post_id ) {
					...
					$icon = get_the_guid( get_post_thumbnail_id( $directory_post_id ) );
					restore_current_blog();
				}

				if ( $icon ) {
					return '<div class="plugin icon"><img src="' . $icon . '" width="' . $size . '" height="' . $size . '"></div>';
				}
	...
	private function get_translation_editors( $project, $locale_slug ) {
		...
		$editors['project'] = (array) $wpdb->get_col( $wpdb->prepare( "
			SELECT
				`user_id`
			FROM {$wpdb->plat_translation_editors}
			WHERE
				`project_id` = %d
	...
	public function get_active_top_level_projects() {
		...
		return GP::$project->many( "
			...
			WHERE
				...
				AND active = 1
			ORDER BY FIELD( slug, 'core', 'themes', 'plugins', 'docs', 'hosting' )
		" );
	}
