...
class WP_Themes extends WP_Directory {
	...
	public function get_theme_projects( $project_slug ) {
		...
		$project_path = 'themes/' . $project_slug;
	...
	public function get_theme_contributors( $project_slug ) {
		...
		$contributors_by_locale = gp_get_meta( 'themes', $project->id, 'contributors-by-locale' );
		if ( ! $contributors_by_locale || $contributors_by_locale['last_updated'] + HOUR_IN_SECONDS < time() ) {
			...
			gp_update_meta( $project->id, 'contributors-by-locale', $contributors_by_locale, 'themes' );
		}

		$chart_data = gp_get_meta( 'themes', $project->id, 'contributors-chart-data' );
		if ( ! $chart_data || $chart_data['last_updated'] + DAY_IN_SECONDS < time() ) {
			...
			gp_update_meta( $project->id, 'contributors-chart-data', $chart_data, 'themes' );
		}
