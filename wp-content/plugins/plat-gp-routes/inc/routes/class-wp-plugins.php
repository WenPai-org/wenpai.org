...
class WP_Plugins extends WP_Directory {
	...
	public function get_plugin_projects( $project_slug ) {
		...
		$project_path = 'plugins/' . $project_slug;
		...
		$has_error = ( ! in_array( 'readme', $sub_projects ) && ! in_array( 'body', $sub_projects ) );
	...
	public function get_plugin_contributors( $project_slug ) {
		...
		$contributors_by_locale = gp_get_meta( 'plugins', $project->id, 'contributors-by-locale' );
		if ( ! $contributors_by_locale || $contributors_by_locale['last_updated'] + HOUR_IN_SECONDS < time() ) {
			...
			gp_update_meta( $project->id, 'contributors-by-locale', $contributors_by_locale, 'plugins' );
		}

		$chart_data = gp_get_meta( 'plugins', $project->id, 'contributors-chart-data' );
		if ( ! $chart_data || $chart_data['last_updated'] + DAY_IN_SECONDS < time() ) {
			...
			gp_update_meta( $project->id, 'contributors-chart-data', $chart_data, 'plugins' );
		}
	...
	private function get_plugin_icon( $project, $size = 64 ) {
		...
		$directory_post_id = gp_get_meta( 'project', $project->id, 'directory_post_id' );
