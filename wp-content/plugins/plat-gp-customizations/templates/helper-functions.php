...
gp_enqueue_style( 'plat-translate' );
...
function wporg_translation_help_modal( $locale ) {
    ...
    <div id="wporg-translation-help-modal" class="wporg-translate-modal">
        <div class="wporg-translate-modal__overlay">
            <div class="wporg-translate-modal__frame" role="dialog"
                 aria-labelledby="wporg-translation-help-modal-headline">
                <div class="wporg-translate-modal__header">
                    ...
                    <button type="button" aria-label="Close modal" class="wporg-translate-modal__close"><span
                                class="screen-reader-text">Close</span><span aria-hidden="true"
                ...
                <div class="wporg-translate-modal__content">
                    <div class="wporg-translate-modal__content_primary">
                        ...
                        <p>After a contributor suggests a string, the string gets a status of “suggested”. In order to
                            transform them into “approved” strings (which will be used in WordPress), a Translation
                            Editor needs to accept (or reject) those suggestions.<br>
                            Please give editors a reasonable time to validate suggestions — We are all volunteers.</p>

                        <h2>Translation Status</h2>
                        <ul class="no-list">
                            <li><span style="color: #c1e1b9">◼︎</span> <strong>Current:</strong> Indicates an approved
                                string.
                            </li>
                            <li><span style="color: #ffe399">◼︎</span> <strong>Waiting:</strong> Indicates a string that
                                was suggested, but not yet approved by a Translation Editor.
                            </li>
                            <li><span style="color: #fbc5a9">◼︎</span> <strong>Fuzzy:</strong> Indicates a “fuzzy”
                                string. Those translations need to be reviewed for accuracy and edited or approved.
                            </li>
                            <li><span style="color: #cdc5e1">◼︎</span> <strong>Old:</strong> Indicates a string that was
                                obsoleted by a newer, approved translation.
                            </li>
                            <li><span style="color: #dc3232">◼︎</span> <strong>Warning:</strong> Indicates validation
                                warnings. These translations either need to be corrected or their warnings explicitly
                                discarded by a Translation Editor.
                            </li>
                        </ul>
                    </div>
                    <div class="wporg-translate-modal__content_secondary">
						<?php if ( $locale_resources ) : ?>
                            <h2><span aria-hidden="true" class="dashicons dashicons-info"></span> Locale Resources
                                for <?php echo esc_html( $locale->english_name ); ?></h2>
							<?php echo wp_kses_post( $locale_resources ); ?>
						<?php endif; ?>

                        <h2><span aria-hidden="true" class="dashicons dashicons-admin-page"></span> Global Handbook
                            Resources</h2>
						<?php
...
function wporg_gp_translate_textarea( $entry, $permissions, $index = 0 ) {
    ...
    <div class="textareas<?php echo ( 0 === $index ) ? ' active' : ''; ?>" data-plural-index="<?php echo $index; ?>">
		...
		if ( isset( $entry->warnings[ $index ] ) ) :
			...
			foreach ( $warnings as $key => $value ) :
                ...
                <div class="warning secondary">
					...
					<?php if ( $can_approve ) : ?>
                        <a href="#" class="discard-warning"
                           data-nonce="<?php echo esc_attr( wp_create_nonce( 'discard-warning_' . $index . $key ) ); ?>"
                           data-key="<?php echo esc_attr( $key ); ?>"
                           data-index="<?php echo esc_attr( $index ); ?>"><?php _e( 'Discard', 'glotpress' ); ?></a>
					<?php endif; ?>
                </div>
			<?php
			endforeach;
		...
		?>
        <textarea placeholder="Enter translation here" class="foreign-text"
                  name="translation[<?php echo esc_attr( $entry->original_id ); ?>][]"
                  id="translation_<?php echo esc_attr( $entry->original_id ); ?>_<?php echo esc_attr( $index ); ?>" <?php echo $disabled; // WPCS: XSS ok. ?>><?php echo esc_translation( gp_array_get( $entry->translations, $index ) ); // WPCS: XSS ok. ?></textarea>
    </div>
	<?php
}
    ...
    <ul>
		...
		foreach ( $entry->references as $reference ) :
            ...
            elseif ( wp_http_validate_url( $reference ) ) :
				?>
                <li><a target="_blank"
                       href="<?php echo esc_url( $reference ); ?>"><?php echo esc_html( $reference ); ?></a></li>
			<?php
			else :
