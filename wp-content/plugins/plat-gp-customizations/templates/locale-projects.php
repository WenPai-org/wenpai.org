...
$breadcrumb   = array();
$breadcrumb[] = gp_link_get( '/', __( '地区' ) );
$breadcrumb[] = gp_link_get( gp_url_join( '/locale', $locale_slug, $set_slug ), esc_html( $locale->native_name ) );
...
<div class="locale-header">
	<p class="locale-intro">您可以通过安装<a href="https://wp-china-yes.com/download" target="_blank" rel="noopener">文派叶子🍃 （WP-China-Yes）</a>插件切换翻译源为「文派开源」，后台可接收全部中文翻译推送（只推送翻译率大于 80% 的包）。</p>
...
<div class="filter-header">
	...
	<div class="search-form">
		<form>
			...
			<label class="screen-reader-text" for="projects-filter"><?php esc_attr_e( '搜索项目...' ); ?></label>
			<input placeholder="<?php esc_attr_e( '搜索项目...' ); ?>" type="search" id="projects-filter" name="s" value="<?php echo esc_attr( $search ?? '' ); ?>" class="filter-search">
			<input type="submit" value="<?php esc_attr_e( '搜索' ); ?>" class="screen-reader-text" />
		</form>
	</div>
</div>
<div class="sort-bar">
	<form id="sort-filter" action="" method="GET">
		...
		<select id="filter" class="is-small" name="filter">
			<?php
				$sorts = array();
				if ( is_user_logged_in() && in_array( $project->slug, array( 'themes', 'plugins' ) ) ) {
		...
		<button type="submit" class="button is-small">应用过滤</button>
	</form>
</div>
...
<div id="projects" class="projects">
	...
	foreach ( $sub_projects as $sub_project ) {
		...
		if ( 'waiting' == $project->slug ) {
			...
			$prefix = 'https://translate.wenpai.org/projects/patterns/core';
		...
		<div class="project <?php echo $classes; ?>">
			<div class="project-top">
				...
				<div class="project-description">
					<p><?php
						...
						echo wp_trim_words( $description, 80 );
					?></p>
				</div>
			</div>
			...
			<div class="project-status">
				...
				<div class="project-status-remaining">
					<span class="project-status-title">剩余</span>
			...
			<div class="project-bottom">
				<?php echo gp_link_get( $project_url, '翻译项目', [ 'class' => 'button contribute-button' ] ); ?>
			</div>
		</div>
		<?php
	}
	if ( ! $sub_projects ) {
		if ( 'waiting' === $project->slug ) {
			echo '<div class="no-projects-found">未找到等待批准的项目！</div>';
		} else {
			echo '<div class="no-projects-found">未找到项目</div>';
		}
	}
	?>
</div>
