...
if ( $translation->translation_status ) {
	...
	$more_links['translation-permalink'] = '<a href="' . esc_url( $translation_permalink ) . '">翻译的固定链接</a>';
} else {
	...
	$more_links['original-permalink'] = '<a href="' . esc_url( $original_permalink ) . '">原文的固定链接</a>';
}
...
$more_links['history'] = '<a href="' . esc_url( $original_history ) . '">翻译历史</a>';
			...
			<div class="editor-panel__left">
				<div class="panel-header">
					...
					<div class="panel-header-actions">
						<button type="button" class="panel-header-actions__cancel with-tooltip" aria-label="关闭当前编辑器">
							<span class="screen-reader-text">关闭</span><span aria-hidden="true" class="dashicons dashicons-no-alt"></span>
						</button>
						<button type="button" class="panel-header-actions__previous with-tooltip" aria-label="打开上一个编辑器">
							<span class="screen-reader-text">上一个</span><span aria-hidden="true" class="dashicons dashicons-arrow-up-alt2"></span>
						</button>
						<button type="button" class="panel-header-actions__next with-tooltip" aria-label="打开下一个编辑器">
							<span class="screen-reader-text">下一个</span><span aria-hidden="true" class="dashicons dashicons-arrow-down-alt2"></span>
						</button>
						<div class="button-menu">
							<button type="button" class="button-menu__toggle with-tooltip" aria-label="显示上下文链接">
								<span class="screen-reader-text">链接</span><span aria-hidden="true" class="dashicons dashicons-menu-alt"></span>
							</button>
							...
							<?php if ( $can_edit ) : ?>
								<div class="translation-actions__primary">
									<button class="button is-primary translation-actions__save with-tooltip"
											type="button"
											aria-label="<?php echo $can_approve_translation ? '保存并批准翻译' : '建议新翻译'; ?>"
											data-nonce="<?php echo esc_attr( wp_create_nonce( 'add-translation_' . $translation->original_id ) ); ?>">
										<?php echo $can_approve_translation ? '保存' : '建议'; ?>
									</button>
								</div>
								<div class="translation-actions__secondary">
									<button type="button" class="button is-small translation-actions__copy with-tooltip" aria-label="复制原文">
										<span class="screen-reader-text">复制</span><span aria-hidden="true" class="dashicons dashicons-admin-page"></span>
									</button>
									<button type="button" class="button is-small translation-actions__insert-tab with-tooltip" aria-label="插入 Tab">
										<span class="screen-reader-text">插入 Tab</span><span aria-hidden="true" class="dashicons dashicons-editor-indent"></span>
									</button>
									<button type="button" class="button is-small translation-actions__insert-newline with-tooltip" aria-label="插入换行">
										<span class="screen-reader-text">插入换行</span><span aria-hidden="true" class="dashicons dashicons-editor-break"></span>
									</button>
									<button type="button" class="button is-small translation-actions__ltr with-tooltip" aria-label="切换到 LTR">
										<span class="screen-reader-text">LTR</span><span aria-hidden="true" class="dashicons dashicons-editor-ltr"></span>
									</button>
									<button type="button" class="button is-small translation-actions__rtl with-tooltip" aria-label="切换到 RTL">
									...
									<!--<button type="button" class="button is-small translation-actions__help with-tooltip" aria-label="Show help">
										<span class="screen-reader-text">帮助</span><span aria-hidden="true" class="dashicons dashicons-editor-help"></span>
									</button>-->
								</div>
							<?php elseif ( is_user_logged_in() ) : ?>
								<?php echo apply_filters( 'wporg_translate_cannot_edit_translation_hint', '<p class="info">道歉，您没有编辑此翻译的权限。</p>', $translation ); ?>
							<?php else : ?>
								<p class="info">
									<?php
									printf(
										'您<a href="%s">必须登录</a>才能编辑此翻译。',
...
<!--				<div class="panel-header">
				...
				<div class="panel-content">
							...
							<div class="status-actions">
								<?php if ( $can_approve_translation ) : ?>
									<?php if ( 'current' !== $translation->translation_status ) : ?>
										<button class="button  is-primary approve" tabindex="-1" data-nonce="<?php echo esc_attr( wp_create_nonce( 'update-translation-status-current_' . $translation->id ) ); ?>"><strong>+</strong> <?php _e( '批准', 'glotpress' ); ?></button>
									<?php endif; ?>
									<?php if ( 'rejected' !== $translation->translation_status ) : ?>
										<button class="button reject" tabindex="-1" data-nonce="<?php echo esc_attr( wp_create_nonce( 'update-translation-status-rejected_' . $translation->id ) ); ?>"><strong>&minus;</strong> <?php _e( '拒绝', 'glotpress' ); ?></button>
										<?php if ( apply_filters( 'gp_enable_changesrequested_status', false ) ) : // todo: delete when we merge the gp-translation-helpers in GlotPress ?>
											<button class="button changesrequested" style="display: none;" data-nonce="<?php echo esc_attr( wp_create_nonce( 'update-translation-status-changesrequested_' . $translation->id ) ); ?>" title="<?php esc_attr_e( '请求对此翻译进行更改，现有翻译将作为翻译历史记录的一部分保留。', 'glotpress' ); ?>"><strong>&minus;</strong> <?php _ex( '请求更改', 'Action', 'glotpress' ); ?></button>
										<?php endif; ?>
									<?php endif; ?>
									<?php if ( 'fuzzy' !== $translation->translation_status ) : ?>
										<button class="button fuzzy" tabindex="-1" data-nonce="<?php echo esc_attr( wp_create_nonce( 'update-translation-status-fuzzy_' . $translation->id ) ); ?>"><strong>~</strong> <?php _e( '模糊', 'glotpress' ); ?></button>
									<?php endif; ?>
								<?php elseif ( $can_reject_self ) : ?>
									<button class="button reject" tabindex="-1" data-nonce="<?php echo esc_attr( wp_create_nonce( 'update-translation-status-rejected_' . $translation->id ) ); ?>"><strong>&minus;</strong> <?php _e( '拒绝原因', 'glotpress' ); ?></button>
									<button class="button fuzzy" tabindex="-1" data-nonce="<?php echo esc_attr( wp_create_nonce( 'update-translation-status-fuzzy_' . $translation->id ) ); ?>"><strong>~</strong> <?php _e( '模糊', 'glotpress' ); ?></button>
								<?php endif; ?>
							</div>
						<?php endif; ?>
					...
					$sidebar_tabs .= '	<li class="current tab-meta" data-tab="sidebar-tab-meta-' . $translation->row_id . '" data-row-id="' . $translation->row_id . '">元数据</li>';
