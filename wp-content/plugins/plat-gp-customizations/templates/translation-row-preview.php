...
<tr class="preview <?php gp_translation_row_classes( $translation ); ?>" id="preview-<?php echo esc_attr( $translation->row_id ); ?>" row="<?php echo esc_attr( $translation->row_id ); ?>">
	...
	<td class="priority" title="<?php echo esc_attr( sprintf( __( '优先级: %s', 'glotpress' ), gp_array_get( GP::$original->get_static( 'priorities' ), $translation->priority ) ) ); ?>">
		<?php
	...
	<td class="original">
		...
		if ( $show_context || $show_priority ) :
			...
			<div class="original-tags">
				...
				if ( $show_priority ) :
					?>
					<span class="priority bubble"><?php echo esc_html( sprintf( '优先级: %s', gp_array_get( GP::$original->get_static( 'priorities' ), $translation->priority ) ) ); ?></span>
					<?php
				endif;
				?>
			</div>
	...
	<td class="translation foreign-text">
		<?php
		if ( $can_edit ) {
			$edit_text = __( '双击添加', 'glotpress' );
		} elseif ( is_user_logged_in() ) {
			$edit_text = __( '您没有权限添加翻译。', 'glotpress' );
		} else {
			/* translators: %s: url */
			$edit_text = sprintf( __( '您<a href="%s">必须登录</a>才能添加翻译。', 'glotpress' ), esc_url( wp_login_url( gp_url_current() ) ) );
		}
		...
		elseif ( $translation->plural && 2 === $locale->nplurals && 'n != 1' === $locale->plural_expression ) :
			...
			foreach( range( 0, $locale->nplurals - 1 ) as $plural_index ):
				...
				?>
				<li>
					<small class="with-tooltip" aria-label="<?php printf('这种复数形式用于数字，例如：%s', $plural_string ); ?>">
