...
class Plugin {
	...
	function plugins_loaded() {
		add_filter( 'gp_tmpl_load_locations', function ( $locations, $template, $args, $template_path ) {
			...
			require_once WP_CONTENT_DIR . '/plugins/plat-gp-customizations/templates/helper-functions.php';
			return [
				WP_CONTENT_DIR . '/plugins/plat-gp-customizations/templates/',
			];
		}, 1, 4 );
		...
		add_filter( 'project_description', array( $this, 'sanitize_project_description' ) );

		// Cron.
		add_filter( 'cron_schedules', [ $this, 'register_cron_schedules' ] );
		add_action( 'init', [ $this, 'register_cron_events' ] );
		add_action( 'plat_translate_update_contributor_profile_badges', [ $this, 'update_contributor_profile_badges' ] );
		add_action( 'plat_translate_update_polyglots_stats', [ $this, 'update_polyglots_stats' ] );
		add_action( 'plat_translate_duplicate_translations', [ $this, 'duplicate_translations' ] );
	 ...
	 */
	public function sanitize_project_description( $description ) {
		return esc_html( gp_html_excerpt( $description, 800 ) );
	}
	...
	public function register_cron_events() {
		if ( ! wp_next_scheduled( 'plat_translate_update_contributor_profile_badges' ) ) {
			wp_schedule_event( time(), '15_minutes', 'plat_translate_update_contributor_profile_badges' );
		}
		if ( ! wp_next_scheduled( 'plat_translate_update_polyglots_stats' ) ) {
			wp_schedule_event( time(), 'daily', 'plat_translate_update_polyglots_stats' );
		}
		if ( ! wp_next_scheduled( 'plat_translate_duplicate_translations' ) ) {
			wp_schedule_event( time(), 'daily', 'plat_translate_duplicate_translations' );
		}
	}
	...
	public function update_contributor_profile_badges() {
		...
		$now       = time();
		$last_sync = get_option( 'plat_translate_last_badges_sync' );
		if ( ! $last_sync ) {
			return;
		}
		...
		if ( ! $user_ids ) {
			update_option( 'plat_translate_last_badges_sync', $now );
			return;
		}
			...
			if ( $existing_rejected_translations ) {
				...
				foreach ( $existing_rejected_translations as $e ) {
					if ( array_pad( $translations, $locale->nplurals, null ) == $e->translations ) {
						GP::$current_route->die_with_error( __( 'Identical rejected translation already exists.', 'plat' ), 200 );
					}
				}
			}
	...
	public function after_setup_theme() {
		register_nav_menu( 'plat_header_subsite_nav', 'WordPress.org Header Sub-Navigation' );
		register_nav_menu( 'plat_translate_global_resources', 'Global Handbook Resources' );
	}
	...
	public function register_cli_commands() {
		WP_CLI::add_command( 'plat-translate duplicate-translations', __NAMESPACE__ . '\CLI\Duplicate_Translations_CLI', $args = [] );
		WP_CLI::add_command( 'plat-translate init-locale', __NAMESPACE__ . '\CLI\Init_Locale' );
		WP_CLI::add_command( 'plat-translate language-pack', __NAMESPACE__ . '\CLI\Language_Pack' );
		WP_CLI::add_command( 'plat-translate mass-create-sets', __NAMESPACE__ . '\CLI\Mass_Create_Sets' );
		WP_CLI::add_command( 'plat-translate make-core-pot', __NAMESPACE__ . '\CLI\Make_Core_Pot' );
		WP_CLI::add_command( 'plat-translate export', __NAMESPACE__ . '\CLI\Export' );
		WP_CLI::add_command( 'plat-translate export-json', __NAMESPACE__ . '\CLI\Export_Json' );
		WP_CLI::add_command( 'plat-translate show-stats', __NAMESPACE__ . '\CLI\Stats_Print' );
	...
	public function update_projects_last_updated() {
		update_option( 'plat_projects_last_updated', time() );
	}

	/**
	 * Returns the profile.wenpai.org URL of a user.
	 *
	...
	public function worg_profile_url( $url, $nicename ) {
		return 'https://profiles.wenpai.org/' . $nicename . '/';
	}
	...
	function localize_links( $content, $wp_locale ) {
		...
		if ( ! isset( $subdomains[ $wp_locale ] ) ) {
			$subdomains[ $wp_locale ] = $wpdb->get_var( $wpdb->prepare( 'SELECT subdomain FROM plat_locales WHERE locale = %s LIMIT 1', $wp_locale ) );
		}
