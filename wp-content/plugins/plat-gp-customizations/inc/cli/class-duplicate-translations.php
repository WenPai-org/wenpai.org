...
class Duplicate_Translations {
	...
	private function notify_duplicates() {
		...
		if ( 0 == count( $this->duplicates ) ) {
			...
			$message .= "Execute `wp wporg-translate duplicate-translations --url=translate.wenpai.org --verbose` to show the duplicate entries.\n\n";
			$message .= "Execute `wp wporg-translate duplicate-translations --url=translate.wenpai.org --fix` to update the duplicate entries, solving the problem.\n\n";
		}
		$message .= $mysql_error;
