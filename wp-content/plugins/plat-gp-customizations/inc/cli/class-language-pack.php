...
class Language_Pack extends WP_CLI_Command {
	...
	private function get_plugin_stable_tag( $plugin_slug ) {
		$plugin = @file_get_contents( "https://api.wpmirror.com/plugins/info/1.0/{$plugin_slug}.json?fields=stable_tag" );
		if ( ! $plugin ) {
			return false;
		}
	...
	private function get_latest_theme_version( $theme_slug ) {
		$theme = @file_get_contents( "https://api.wpmirror.com/themes/info/1.1/?action=theme_information&request[slug]={$theme_slug}" );
		if ( ! $theme ) {
			return false;
		}
	...
	private function get_latest_plugin_version( $plugin_slug ) {
		$plugin = @file_get_contents( "https://api.wpmirror.com/plugins/info/1.0/{$plugin_slug}.json" );
		if ( ! $plugin ) {
			return false;
		}
