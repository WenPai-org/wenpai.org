...
define( 'wpcs_DEBUG', true );
$wpcs_deubg_data = array(); //uncomment this line to enable debug
...
function wpcs_add_global_js() {
	...
	wp_register_script( 'wpcs-block-script-ok', plugins_url( '/assets/js/wpcs-block-script-ok.js', __FILE__ ), array(
		'wp-blocks',
		'wp-element',
		'wpcs-variant'
	), '1.3.0' );
...
if ( file_exists( $modules_dir . 'wpcs-wp-super-sitemap.php' ) ) {
	require_once $modules_dir . 'wpcs-wp-super-sitemap.php';
}
...
$wpcs_langs = array(
	'zh-cn' => array( 'zhconversion_cn', 'cntip', __( '简体中文', 'wpchinese-switcher' ), 'zh-CN' ),
	'zh-tw' => array( 'zhconversion_tw', 'twtip', __( '台灣正體', 'wpchinese-switcher' ), 'zh-TW' ),
	'zh-hk' => array( 'zhconversion_hk', 'hktip', __( '港澳繁體', 'wpchinese-switcher' ), 'zh-HK' ),
...
if ( is_multisite() && wpcs_mobile_exist( 'network' ) ) {
	add_action( 'network_admin_menu', 'wpcs_admin_init' );
} else {
	add_action( 'admin_menu', 'wpcs_admin_init' );
}
...
function wpcs_init() {
	...
	if ( isset( $wpcs_options['wpcs_use_permalink'] ) && $wpcs_options['wpcs_use_permalink'] != 0 && empty( $wp_rewrite->permalink_structure ) ) {
		$wpcs_options['wpcs_use_permalink'] = 0;
		update_wpcs_option( 'wpcs_options', $wpcs_options );
	}
...
function wpcs_link_conversion( $link, $variant = null ) {
	global $wpcs_options, $wpcs_modules;
	...
	if ( str_contains( $link, '?' ) || ! $wpcs_options['wpcs_use_permalink'] ?? 0 ) {
	...
	return str_replace( $wpcs_wp_home, "$wpcs_wp_home/$variant", $link );
...
function my_plugin_enqueue_block_editor_assets() {
	global $wpcs_options;
	set_wpcs_langs_urls();
	wp_register_script( 'wpcs-block-script', plugins_url( '/assets/js/gudengbao.js', __FILE__ ), array(
		'wp-blocks',
		'wp-element',
		'wp-block-editor'
	), '1.2.0' );
	...
	wp_localize_script( 'wpcs-block-script', 'wpc_switcher_navi_data', array(
		'wpcs_navi' => $aProps,
		'type_arr'  => $wpcs_translate_type
	) );
}
...
function wpcs_activate() {
	$current_options = (array) get_wpcs_option( 'wpcs_options' );
	$wpcs_options    = array(
		'wpcs_search_conversion'       => 1,
		'wpcs_used_langs'              => array( 'zh-hans', 'zh-hant', 'zh-cn', 'zh-hk', 'zh-sg', 'zh-tw' ),
		'wpcs_browser_redirect'        => 0,
...
add_action( 'rest_api_init', function () {
	...
	register_rest_route( 'wpchinese-switcher/v1', '/get_style_html/', array(
		'methods'             => 'GET',
		'callback'            => function () {
			$done = function () {
				echo wpcs_output_navi( '', true );
				exit();
			};
...
add_action( 'init', function () {
	...
	register_block_type( 'wpcs/gutenberg-castle', array(
...
function add_suffix_to_links( $html ) {
	...
	$result = preg_replace_callback( $pattern, function ( $matches ) {
		...
		// 转换链接
		$new_href = wpcs_link_conversion( $href );
...
// 封装获取配置的函数
function get_wpcs_option( $option_name ) {
	global $wpcs_modules;
	if ( is_multisite() && wpcs_mobile_exist( 'network' ) ) {
...
// 封装更新配置的函数
function update_wpcs_option( $option_name, $option_value ) {
	global $wpcs_modules;
	if ( is_multisite() && wpcs_mobile_exist( 'network' ) ) {
...
function wpcs_mobile_exist( $name ) {
	$modules_dir = __DIR__ . '/module/';

	return file_exists( $modules_dir . "wpcs-wp-super-$name.php" );
}
