...
class Cross_Locale_PTE {
	...
	public static function get_all_users() {
		...
		$rows = $wpdb->get_results( $wpdb->prepare( "
			...
			{$wpdb->plat_translation_editors} te
			JOIN {$wpdb->gp_projects} p ON te.project_id = p.id
			WHERE te.locale = %s
		", self::ALL_LOCALES ) );
	...
	public static function user_has_cross_locale_permission( $user, $project_id ) {
		...
		$result = $wpdb->get_col( $wpdb->prepare( "
			...
			{$wpdb->plat_translation_editors} te
			JOIN {$wpdb->gp_projects} p ON ( te.project_id = p.id OR te.project_id = p.parent_project_id )
