...
if ( ! class_exists( 'GP_Locales' ) ) {
	require_once WP_CONTENT_DIR . '/plugins/glotpress/locales/locales.php';
}
...
class Rosetta_Roles {
    ...
    public $translation_editors_page;
	...
	public function __construct() {
		...
		$wpdb->plat_translation_editors = self::TRANSLATION_EDITORS_TABLE;
	}
	...
	public function plugins_loaded() {
		...
		add_action( 'translation_editor_added', array( $this, 'update_plat_profile_badge' ) );
		add_action( 'translation_editor_removed', array( $this, 'update_plat_profile_badge' ) );
        ...
        // 取消加载 Cross_Locale_PTE，因为我们用不到
		//Cross_Locale_PTE::init_admin();
	}

    public function register_roles() {
        add_role(
            self::TRANSLATION_EDITOR_ROLE,
            __( 'Translation Editor', 'rosetta' ),
            array(
                'read'         => true,
                'edit_posts'   => true,
                'upload_files' => true,
        ...
        add_role(
            self::GENERAL_TRANSLATION_EDITOR_ROLE,
            __( 'General Translation Editor', 'rosetta' ),
            array(
                'read'         => true,
                'edit_posts'   => true,
                'upload_files' => true,
	...
	private function remove_translation_editor( $user ) {
		...
		$wpdb->query( $wpdb->prepare( "
			DELETE FROM {$wpdb->plat_translation_editors}
	...
	private function update_translation_editor( $user, $projects = array() ) {
		...
		if ( $values_to_add ) {
			$wpdb->query( "
				INSERT INTO {$wpdb->plat_translation_editors}
	 ...
	 */
	public function update_plat_profile_badge( $user ) {
		global $wpdb;
		...
		if ( 'remove' === $action ) {
			$count = (int) $wpdb->get_var(
				$wpdb->prepare(
					"SELECT COUNT(*) FROM {$wpdb->plat_translation_editors}
...
Welcome to the WordPress Polyglots team and happy translating!',
			...
			$message = str_replace(
				...
				[
					...
					'https://translate.wenpai.org/locale/' . $this->gp_locale->slug,
...
Welcome to the WordPress Polyglots team and happy translating.',
			...
			foreach ( $projects_added as $project_id ) {
				if ( $projects[ $project_id ] ) {
					...
					$name = sprintf(
						'%s: %s',
						$name,
						esc_url( 'https://translate.wenpai.org/projects/' . $projects[ $project_id ]->path )
					);
	...
	private function render_edit_translation_editor( $user_id ) {
		...
		$last_updated = get_blog_option( SITE_ID_TRANSLATE, 'plat_projects_last_updated' );
