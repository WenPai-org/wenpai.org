( function ( $ ) {
	...
	const summarizeCommentThread = ( $commentThread, $summary ) => {
		...
		$avatars.each( function () {
            ...
            $avatar.attr('src', $avatar.attr('src').replace('https://gravatar.com', 'https://cravatar.cn'));
...
$avatars.each(function () {
    const $avatar = $(this).clone().removeClass('grav-hijack');
    let gravatarHash = '';
    const match = $avatar.attr('src').match(/.*avatar\/([a-zA-Z0-9]{8})/) || [null, null];
    if (match[0] && match[1]) {
        gravatarHash = match[1];
        $avatar.addClass(`gravatar-${gravatarHash}`);
    }
    $avatar.attr('src', $avatar.attr('src').replace('https://gravatar.com', 'https://cravatar.cn'));
    const avatarHtml = $('<div/>').append($avatar).html();
    const $commentSummaryAvatars = $summary.find('.p2020-comments-summary-avatars');
    if (gravatarHash === '' || $commentSummaryAvatars.find(`img.gravatar-${gravatarHash}`).length < 1) {
        $commentSummaryAvatars.append(avatarHtml);
    }
});
